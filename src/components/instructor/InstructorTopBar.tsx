...
const navItems = [
  { label: 'Meus cursos', to: '/instructor/cursos' },
  { label: 'Inscrições', to: '/instructor/inscricoes' },
];

export const InstructorTopBar = () => {
    ...
    <>
      <style>{`
        @media (max-width: 720px) {
          .purple-instructor-topbar-inner {
            align-items: flex-start !important;
            flex-direction: column !important;
            padding: 12px 20px !important;
          }
          .purple-instructor-topbar-nav {
            width: 100% !important;
            overflow-x: auto !important;
          }
          .purple-instructor-topbar-actions {
            width: 100% !important;
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 20, borderBottom: `1px solid ${palette.brand.subtle}`, background: 'color-mix(in srgb, var(--bg-surface) 94%, transparent)', backdropFilter: 'blur(14px)', fontFamily: typography.fontFamily }}>
        <div className="purple-instructor-topbar-inner" style={{ maxWidth: 1180, minHeight: 68, margin: '0 auto', padding: `0 ${spacing.xl}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: spacing.xl }}>
          <NavLink to="/instructor/cursos" style={{ display: 'flex', alignItems: 'center', gap: spacing.sm, color: 'var(--text-primary)', textDecoration: 'none', flexShrink: 0, fontSize: typography.size.md }}>
            <BrandLogo size="sm" compact />
            <span style={{ color: 'var(--text-muted)', fontWeight: typography.weight.medium }}>Painel do Docente</span>
          </NavLink>

          <nav className="purple-instructor-topbar-nav" aria-label="Navegação do docente" style={{ display: 'flex', alignItems: 'center', gap: spacing.xs, flex: 1 }}>
            {navItems.map((item) => (
          ...
          <div className="purple-instructor-topbar-actions" style={{ display: 'flex', alignItems: 'center', gap: spacing.md, flexShrink: 0 }}>
