...
export const AdminBreadcrumb = ({ items }: AdminBreadcrumbProps) => (
  <nav aria-label="Navegação estrutural" style={{ display: 'flex', alignItems: 'center', gap: spacing.sm, color: 'var(--text-muted)', fontFamily: typography.fontFamily, fontSize: typography.size.sm, flexWrap: 'wrap' }}>
    <NavLink to="/admin/cursos" style={{ color: 'var(--text-secondary)', textDecoration: 'none', padding: `${spacing.xs} ${spacing.sm}`, borderRadius: radius.sm, fontWeight: typography.weight.medium }}>
      Administração
    </NavLink>
